...
def generate_data(mu, cov):
    # 二次元の正規分布を生成
    d = np.random.multivariate_normal(mu, cov)
    return d
...
rate1 = int(np.max([rate, 1-rate]) * step)
rate2 = step - rate1
mu_true_1 = np.array([0.5, 0.5])  # 生成元の平均パラメータ
cov_true_1 = np.array([[0.0025, 0], [0, 0.0025]])  # 生成元の分散パラメータ
mu_true_2 = np.array([0.15, 0.35])  # 生成元の平均パラメータ
cov_true_2 = np.array([[0.0025, 0], [0, 0.0025]])  # 生成元の分散パラメータ
...
for i in range(step):
    if np.random.rand() <= 0.9:
        d = generate_data(mu_true_1, cov_true_1)
    else:
        d = generate_data(mu_true_2, cov_true_2)
    #ebi.Min(d[:, 1])
    ebi.Equal(d)
    #ebi.Rand(d)
"""
...
ax1.scatter(*mu_true_1, color="orange", s=50, zorder=3, label="Center of date generating distribution")
ax1.scatter(*mu_true_2, color="cyan", s=50, zorder=3, label="Center of date generating distribution")
